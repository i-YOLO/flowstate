...
## [2026-01-14 00:00]
### 🛠 数据库兼容性增强 (Database Compatibility)
- **版本降级**: 将 PostgreSQL 镜像版本从 `alpine` (最新版) 降级至 **`12-alpine`**。
- **验证协议适配**: 解决了旧版可视化工具不支持 SCRAM-SHA-256 认证的问题，确保本地开发工具能够顺畅连接数据库。
- **环境重置**: 重新部署数据库容器并清理了旧卷，确保版本切换后的数据一致性。
