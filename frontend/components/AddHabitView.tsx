    ...
    <div className="bg-background-light dark:bg-background-dark text-slate-900 dark:text-white min-h-screen font-display flex flex-col">
      <div className="max-w-[480px] w-full mx-auto min-h-screen flex flex-col bg-background-light dark:bg-background-dark pb-24">
          ...
          <div className="grid grid-cols-4 gap-3 p-4">
            {icons.map((icon) => (
              <button 
                ...
                className={`flex flex-col items-center justify-center aspect-square rounded-xl transition-all duration-200 ${
                  selectedIcon === icon
              ...
              >
                <span className={`material-symbols-outlined text-3xl ${
                  selectedIcon === icon ? 'text-primary' : 'text-slate-600 dark:text-white'
                }`}>
                  {icon}
                </span>
              </button>
            ...
            <div className="flex gap-4 overflow-x-auto no-scrollbar pb-2 px-1 pl-3 pt-2">
              {colors.map((bg) => (
                <button 
                  ...
                  className={`size-10 rounded-full ${bg} shrink-0 transition-all duration-200 ${
                    selectedColor === bg 
                ...
                {days.map((d, i) => {
                    ...
                    return (
                        <button 
                            key={i} 
                            onClick={() => toggleDay(i)}
                            className={`size-11 rounded-full flex items-center justify-center font-bold text-sm transition-all duration-200 ${
                                isSelected 
        ...
        <footer className="fixed bottom-0 left-0 right-0 max-w-[480px] mx-auto p-4 bg-background-light/95 dark:bg-background-dark/95 backdrop-blur-sm">
            <button onClick={onSave} className="w-full bg-primary hover:bg-blue-700 text-white font-bold py-4 rounded-xl shadow-lg shadow-primary/25 transition-all active:scale-[0.98]">
                创建习惯
            </button>
        </footer>
      </div>
